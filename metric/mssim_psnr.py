...
def calculate_mssim_psnr(data_loader, generator):
    psnr, mssim = [], []
    for batch_src, batch_gen in tqdm(
        zip(data_loader, generator), total=len(data_loader)
    ):
        src_imgs, src_img_lens = (
            batch_src["org_imgs"],
            batch_src["org_img_lens"],
        )
        gen_imgs, gen_img_lens = (
            batch_gen["org_imgs"],
            batch_gen["org_img_lens"],
        )
        for src_img, src_img_len, gen_img, gen_img_len in zip(
            src_imgs, src_img_lens, gen_imgs, gen_img_lens
        ):
            ...
            if gen_img_len != src_img_len:
                print(
                    f"gen_img_len {gen_img_len} != src_img_len {src_img_len}"
                )
                show_image_pair(
                    src_img[0, :, :src_img_len].cpu().numpy(),
                    gen_img[0, :, :gen_img_len].cpu().numpy(),
                    "src_img",
                    "gen_img",
                )
            src_img = (
                src_img[:, :, :src_img_len].permute(1, 2, 0).cpu().numpy() + 1
            ) / 2
            gen_img = (
                gen_img[:, :, :gen_img_len].permute(1, 2, 0).cpu().numpy() + 1
            ) / 2
