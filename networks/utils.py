...
from lib.path_config import IMG_HEIGHT, CHAR_WIDTH
...
def get_init_state(
    deepth, batch_size, hidden_dim, device, bidirectional=False
):
    ...
    h0_encoder_bi = torch.zeros(
        deepth, batch_size, hidden_dim, requires_grad=False
    )
...
def words_to_images(texts, img_h, img_w, n_channel=1):
    n_channel = 3
    word_imgs = np.zeros((len(texts), img_h, img_w, n_channel)).astype(
        np.uint8
    )
    for i, text in enumerate(texts):
        # cv2.putText(word_imgs[i], texts[i], (2, 29), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 255, 2)
        word_imgs[i] = pil_text_img(word_imgs[i], text, (1, 1), textSize=25)
    word_imgs = word_imgs.sum(axis=-1, keepdims=True).astype(np.uint8)
    word_imgs = (
        torch.from_numpy(word_imgs).permute([0, 3, 1, 2]).float() / 128 - 1
    )
    return word_imgs
...
class PatchSampler:
    def __init__(
        self, patch_size=(32, 32), sample_density=2, char_size=(64, 32)
    ):
        self.patch_size = (
    ...
    def random_sample(self, fake_imgs, img_lens, ret_xy=False):
        ...
        for bid in range(fake_imgs.size(0)):
            ...
            rand_top_xy = np.random.random(
                (lb_lens[bid] * self.sample_density, 2)
            )
            ...
            for tx, ty in rand_top_xy:
                ...
                patch = fake_imgs[
                    bid,
                    :,
                    ty : ty + self.patch_size[0],
                    tx : tx + self.patch_size[1],
                ]
...
def extract_patches_2d(img, patch_shape, step=[1.0, 1.0], batch_first=False):
    ...
    step_int = [0, 0]
    step_int[0] = (
        int(patch_H * step[0]) if (isinstance(step[0], float)) else step[0]
    )
    step_int[1] = (
        int(patch_W * step[1]) if (isinstance(step[1], float)) else step[1]
    )
...
def extract_all_patches(
    org_imgs, org_img_lens, block_size=32, step=8, plot=False
):
    img_h = org_imgs.size(-2)
    n_patch_row = (img_h - block_size) // step + 1
    patches = extract_patches_2d(
        org_imgs, (block_size, block_size), step=[step, step], batch_first=True
    )
    patch_lens = (org_img_lens - block_size) // step + 1
    mask = (
        _len2mask(patch_lens, patches.size(1) // n_patch_row)
        .repeat(1, n_patch_row)
        .bool()
    )
    ...
    if plot:
        ...
        plt.subplot(211)
        plt.imshow(
            org_imgs[idx, 0, :, : org_img_lens[idx].cpu().detach().numpy()]
            .cpu()
            .detach()
            .numpy(),
            cmap="binary",
        ...
        plt.subplot(212)
        sum_patch_lens = list(
            accumulate(patch_lens.cpu().detach().numpy() * n_patch_row)
        )
        ...
        img = draw_image(
            1 - torch.stack(patch_imgs, dim=0).repeat(1, 3, 1, 1).cpu(),
            nrow=patch_lens[idx],
            normalize=True,
        )
...
def rand_clip_images(imgs, img_lens, min_clip_width=64):
    ...
    for i, (clip_img, clip_img_len) in enumerate(
        zip(clip_imgs, clip_img_lens)
    ):
        pad_imgs[i, 0, :, :clip_img_len] = clip_img
    return torch.from_numpy(pad_imgs).float().to(device), torch.Tensor(
        clip_img_lens
    ).int().to(device)
...
def augment_images(imgs, img_lens, lbs, lb_lens):
    ...
    for img_len in img_lens:
        ...
        ref_img_lens.append(_recalc_len(new_width, scale=CHAR_WIDTH))
    ...
    for i, (img, img_len, ref_img_len) in enumerate(
        zip(imgs.detach(), img_lens, ref_img_lens)
    ):
        mode = "area" if img_len > ref_img_len else "bilinear"
        align_corners = None if img_len > ref_img_len else False
        resized_img = F.interpolate(
            img[:, :, :img_len].unsqueeze(dim=0),
            (h, ref_img_len),
            mode=mode,
    ...
    resized_img_lens = (
        torch.from_numpy(ref_img_lens).int().to(imgs.device).detach()
    )
...
def rescale_images(imgs, img_lens, ref_img_lens):
    ...
    pad_imgs = -np.ones((bz, c, h, _recalc_len(ref_img_lens.max(), h)))
    for i, (img, img_len, ref_img_len) in enumerate(
        zip(imgs, img_lens, ref_img_lens)
    ):
        mode = "area" if img_len > ref_img_len else "bilinear"
        align_corners = None if img_len > ref_img_len else False
        resized_img = F.interpolate(
...
def rescale_images2(imgs, img_lens, lb_lens, ref_img_lens, ref_lb_lens):
    ...
    resized_imgs, target_img_lens = rescale_images(
        imgs, img_lens, target_img_lens.int()
    )
    return resized_imgs, target_img_lens


def pad_image_lengths(img_lens: torch.Tensor, scale=IMG_HEIGHT):
    pad_img_lens = [
        _recalc_len(img_len, scale) for img_len in img_lens.detach()
    ]
