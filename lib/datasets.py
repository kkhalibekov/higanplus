...
from lib.path_config import data_roots, data_paths, IMG_HEIGHT, CHAR_WIDTH
...
class Hdf5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = "".join(
            chr(ch) for ch in self.lbs[lb_seek_idx : lb_seek_idx + lb_len]
        )
        data["text"] = text
        ...
        data["org_img"] = self.org_transforms(
            Image.fromarray(deepcopy(img), mode="L")
        )

        # style image
        if self.process_style:
            h, w = img.shape[:2]
            new_w = CHAR_WIDTH * len(text)
            dim = (new_w, IMG_HEIGHT)
            if new_w < w:
                style_img = cv2.resize(
                    deepcopy(img), dim, interpolation=cv2.INTER_AREA
                )
            else:
                style_img = cv2.resize(
                    deepcopy(img), dim, interpolation=cv2.INTER_LINEAR
                )
    ...
    @staticmethod
    def _recalc_len(leng, scale=CHAR_WIDTH):
        tmp = leng % scale
    ...
    def collect_fn(batch):
        ...
        pad_org_imgs = -np.ones(
            (bz, 1, org_imgs[0].shape[-2], pad_org_img_max_len)
        )
        for i, (org_img, org_img_len) in enumerate(
            zip(org_imgs, org_img_lens)
        ):
        ...
        pad_style_imgs = -np.ones(
            (bz, 1, style_imgs[0].shape[-2], pad_style_img_max_len)
        )
        for i, (style_img, style_img_len) in enumerate(
            zip(style_imgs, style_img_lens)
        ):
        ...
        if len(aug_imgs) > 0:
            pad_aug_imgs = -np.ones(
                (bz, 1, aug_imgs[0].shape[-2], max(aug_img_lens))
            )
    ...
    def merge_batch(batch1, batch2, device):
        ...
        mbdata = {}
        for img_key, img_len_key in [
            ("org_imgs", "org_img_lens"),
            ("style_imgs", "style_img_lens"),
            ("aug_imgs", "aug_img_lens"),
        ]:
            if img_len_key not in batch1:
                continue
            ...
            pad_imgs = (
                -torch.ones(
                    (bz1 + bz2, imgs1.size(1), imgs1.size(2), max_img_len)
                )
                .float()
                .to(device)
            )
    ...
    def gen_h5file(all_imgs, all_texts, all_wids, save_path):
        ...
        h5f.create_dataset("lbs", data=save_lbs, dtype=np.int32)
        ...
        h5f.create_dataset("wids", data=all_wids, dtype=np.int16)
        h5f.close()
...
class ImageDataset(Hdf5Dataset):
    ...
    def _load_h5py(self, file_path, normalize_wid=True):
        ...
        for extension in file_extensions:
            files.extend(glob.glob(os.path.join(file_path, "*." + extension)))
        ...
        for fn in files:
            ...
            new_w = max(int(w * r), int(IMG_HEIGHT / 4 * len(label_text)))
            dim = (new_w, IMG_HEIGHT)
            if new_w < w:
                resize_img = cv2.resize(img, dim, interpolation=cv2.INTER_AREA)
            else:
                resize_img = cv2.resize(
                    img, dim, interpolation=cv2.INTER_LINEAR
                )
        ...
        self.img_seek_idxs, self.lb_seek_idxs = np.array(img_seek_idxs).astype(
            np.int64
        ), np.array(lb_seek_idxs).astype(np.int64)
        self.img_lens, self.lb_lens = np.array(img_lens).astype(
            np.int32
        ), np.array(lb_lens).astype(np.int32)
        self.wids = np.zeros((len(all_imgs),)).astype(np.int32)


def get_dataset(
    dset_name, split, wid_aug=False, recogn_aug=False, process_style=False
):
