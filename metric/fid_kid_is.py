...
def get_activations(data_source, n_batches, model, dims, device, crop=False):
    ...
    for batch in tqdm(data_source, total=n_batches):
        imgs, org_img_lens = batch["org_imgs"].to(device), batch[
            "org_img_lens"
        ].to(device)
...
def polynomial_mmd_averages(
    ...
    **kernel_args,
    ...
    if subset_size > len(codes_g):
        print(
            (
                "Warning: subset_size is bigger than len(codes_g). [sub:{}  code_g:{}]".format(
    ...
    if subset_size > len(codes_r):
        print(
            f"Warning: subset_size is bigger than len(codes_r). [sub:{subset_size}  code_g:{len(codes_r)}]"
        )
...
def calculate_fid_kid_is(
    cfg, data_loader, generator, n_rand_repeat, device, crop=False
):
    """
    ATTENTION:
        the backgroud value of input images must be -1, and the foreground values should be less than 1.
    """
    ...
    with torch.no_grad():
        act2, m2, s2, logits2 = calculate_activation_statistics(
            generator,
            n_batches * n_rand_repeat,
            inceptionV3_model,
            cfg.dims,
            device,
            crop,
        )
        act1, m1, s1, logits1 = calculate_activation_statistics(
            data_loader, n_batches, inceptionV3_model, cfg.dims, device, crop
        )
