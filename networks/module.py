...
from networks.block import Conv2dBlock, ActFirstResBlock, DeepBLSTM, DeepLSTM
...
class StyleBackbone(nn.Module):
    def __init__(
        ...
        cnn_f = [
            ...
            Conv2dBlock(
                in_channel, nf, 5, 2, 0, norm="none", activation="none"
            ),
        ]
        for i in range(2):
            ...
            cnn_f += [
                ActFirstResBlock(
                    nf, nf, None, "relu", norm, "zero", dropout=dropout / 2
                )
            ]
            cnn_f += [nn.ZeroPad2d((1, 1, 0, 0))]
            cnn_f += [
                ActFirstResBlock(
                    nf, nf_out, None, "relu", norm, "zero", dropout=dropout / 2
                )
            ]
        ...
        for i in range(2):
            ...
            cnn_f += [
                ActFirstResBlock(
                    df, df, None, "relu", norm, "zero", dropout=dropout
                )
            ]
            cnn_f += [
                ActFirstResBlock(
                    df, df_out, None, "relu", norm, "zero", dropout=dropout
                )
            ]
            if i < 1:
                cnn_f += [nn.MaxPool2d(kernel_size=3, stride=2)]
...
class StyleEncoder(nn.Module):
    ...
    def forward(
        self, img, img_len, cnn_backbone=None, ret_feats=False, vae_mode=False
    ):
        feat, all_feats = cnn_backbone(img, ret_feats)
        img_len = img_len // cnn_backbone.reduce_len_scale
        img_len_mask = (
            _len2mask(img_len, feat.size(-1)).unsqueeze(1).float().detach()
        )
        style = (feat * img_len_mask).sum(dim=-1) / (
            img_len.unsqueeze(1).float() + 1e-8
        )
...
class WriterIdentifier(nn.Module):
    ...
    def forward(self, img, img_len, cnn_backbone, ret_feats=False):
        ...
        wid_feat = (feat * img_len_mask).sum(dim=-1) / (
            img_len.unsqueeze(1).float() + 1e-8
        )
    ...
    def return_feat(self, img, img_len):
        ...
        img_len_mask = (
            _len2mask(img_len, out_w.size(-1)).unsqueeze(1).float().detach()
        )
        wid_feat = (out_w * img_len_mask).sum(dim=-1) / (
            img_len.unsqueeze(1).float() + 1e-8
        )
        for j in range(2):
